...
struct TravaMapView: UIViewRepresentable {
    ...
    final class Coordinator: NSObject, MKMapViewDelegate {
        ...
        func updateOverlays(
            ...
            if showHeatmap {
                // Remove path overlays, show heatmap.
                let toRemove = map.overlays.filter { !($0 is HeatmapOverlay) }
                map.removeOverlays(toRemove)
                currentPolyline = nil
                renderedTrackIds = []

                // Rebuild whenever allTracks count or intensity changes.
                // allTracks is regenerated by refreshTracks() on every explicit
                // refresh call, so a changed count always means fresh data.
                ...
                // Remove heatmap, show path overlays.
                if let h = heatmapOverlay {
                    map.removeOverlay(h)
                    heatmapOverlay        = nil
                    lastHeatmapTrackCount = 0   // force full rebuild next time heatmap is toggled on
                }

                // Append newly-seen tracks without removing existing overlays (no flicker).
                // If tracks were cleared (filter change), completedTracks will be a smaller
                // set — detect this by checking if any rendered ID is no longer present.
                let currentIds = Set(completedTracks.map { $0.trackId })
                if !renderedTrackIds.isSubset(of: currentIds) {
                    // Filter changed: remove all path overlays and re-add the current set.
                ...
                // Update live recording path (remove old, add new).
